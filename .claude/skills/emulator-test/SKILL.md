...
- [ ] 🔴 **예약이 7개보다 적을 수 있다(1.6.0~).** 간격 반복이 붙으면서 *그날 볼 것이 없는 날은
      예약하지 않는다.* 방금 전부 복습한 상태면 며칠간 0개가 정상이다 — "7개가 아니면 고장"으로
      읽으면 안 된다. 만기를 확인하려면 단어를 새로 추가해 본다(안 푼 단어는 언제나 만기다)
- [ ] 알림 제목에 **숫자**가 들어가나(`오늘 복습할 단어 N개`). 옛 문구 `오늘 단어 복습할까요?` 가
      보이면 구버전이 깔려 있는 것이다

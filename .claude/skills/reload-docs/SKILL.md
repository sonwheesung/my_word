...
(`common/GAME_ASSET_SOURCING.md` §3.3).

### ⚠ ② 는 원래 **날짜 대조 두 줄**이었다. 그게 못 잡는 것이 있다 (2026-09-11 정정)

```bash
# ~~옛 ②~~ 🚫 이걸로 판정하지 마라
git log -1 --format=%ad --date=short -- docs/open-source-licenses.html
git log -1 --format=%ad --date=short -- package.json     # 이쪽이 최근이면 낡았다
```

이 대조는 **의존성이 바뀌었나**만 본다. 그런데 고지에는 **앱 버전도 함께 박힌다**
(*"My Word 1.6.0 기준"*). 의존성을 안 건드리고 버전만 올리면 두 날짜는 **영원히 같고**,
고지만 조용히 낡는다. **구조적으로 못 보는 자리이지 운이 나빴던 것이 아니다.**

🔴 **2026-09-11 실측**: 날짜는 둘 다 `2026-09-07` 이라 **초록으로 읽혔다.**
같은 자리에서 `npm run check:licenses` 는 **FAIL** 이었다.

```
패키지 개수   218 vs 218      ← 목록은 멀쩡했다. 그래서 개수로도 못 잡는다
어긋난 것     "My Word 1.4.0 기준"   실제는 1.6.0 (두 릴리스 낡음)
```

→ **고지는 의존성을 바꿀 때만이 아니라 `app.json` 버전을 올릴 때도 다시 굽는다.**
→ 가드(`scripts/check-licenses.mjs`)는 생성기를 실제로 돌려 **바이트로** 댄다. 그것만 믿는다.

★ 이 스킬이 *"기계로 대조하라"* 고 적어 놓고 **정작 ② 에는 사람이 눈으로 보는 규칙을 적어
두었다.** 가드는 2026-09-07 에 이미 만들어져 있었는데 이 문서가 그걸 안 가리켰다.
**도구를 만들면 그 도구를 가리키는 문서까지가 일이다.**

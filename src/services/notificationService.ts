 ...
 *
 * ⚠ 같은 id 로 다시 부를 때 무엇이 바뀌고 무엇이 안 바뀌는지가 다르다:
 *   - **이름·설명은 갱신된다.** 그래서 재예약마다 부르는 이 함수가 채널 이름을 현재 앱 언어로
 *     맞춰 준다 — 한국어로 쓰다 영어로 바꾸면 시스템 설정의 채널 이름도 따라간다.
 *   - **중요도·소리·진동은 안 바뀐다.** 사용자가 시스템 설정에서 조정한 것을 앱이 되돌리지
 *     못하게 하는 안드로이드의 의도된 동작이다.
 */

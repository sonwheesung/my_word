...
// --- 플래시카드 ---
// 카드 보기 취향(순서·앞면·발음 자동재생·마지막 카테고리). 2026-09-10 신설.
// 🔴 **새 키다. 기존 키를 건드리지 않는다.** 운영 중인 앱이라 스키마를 늘리지 않고 키만 더한다.
// ⚠ 백업에 담지 않는다. 기기별 취향이라 복원 뒤에 다른 기기의 취향이 덮이면 이상하다
//   (`BACKUP_KEYS` 에 일부러 넣지 않았다).
// 🔴 이 키에는 **학습 기록이 들어가지 않는다.** 플래시카드는 채점하지 않으므로 남길 것이 없고,
//   그래서 정답률과 복습 만기가 카드 때문에 움직이는 일이 없다.
export const FLASHCARD_PREFS_KEY = '@my_word_flashcard_prefs';

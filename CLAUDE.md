...
- 3차: 화면 · 서비스 · 컴포넌트 · 저장 키를 더하거나 뺐으면 `npm run check:docs` (설계 문서 목록 대조. 2026-09-18 신설)
  문서 전체 정합은 `.claude/skills/doc-consistency`. 그날 설계 문서가 9개 화면(실제 15개)에 멈춰 있었다
...
### 🃏 플래시카드 (2026-09-10 구현 · **1.7.0 으로 나간다**)

홈 첫 줄 세 번째 칸. 앞면과 뒷면을 뒤집어 보고 좌우로 넘기는 모드다.
파일은 `flashcardService.ts` · `FlashcardSetupScreen` · `FlashcardScreen` · `FlipCard` 넷이다.

#### 🔴 채점하지 않는 것이 이 모드의 정체다. 이 줄이 이 절의 전부다

**플래시카드는 퀴즈 결과를 한 건도 쓰지 않는다.** 정답률·스트릭·통계·간격 복습 만기가
전부 `@my_word_quiz_results` 에서 파생되므로, 여기서 한 번이라도 쓰면 **그 넷이 동시에 흔들린다.**

⚠ **이건 "아직 안 만든 기능"이 아니라 결정이다**(2026-09-10 사용자 결정: *"점수 안 남긴다"*).
카드를 넘기는 것은 *"봤다"* 이지 *"맞혔다"* 가 아니다. 뒤집어 보고 넘긴 것을 정답으로 세면
정답률이 조용히 부풀고, 그 숫자는 사장님이 자기 학습 상태를 판단하는 근거다.

🔴 **다음 사람이 "점수도 남기면 좋지 않나"로 되돌리기 쉬운 자리라 기계로 막아 뒀다.**
`__tests__/flashcardService.test.ts` 의 마지막 describe 가 **네 파일의 소스를 직접 읽어**
`recordAnswers`·`saveResults`·`quizResultStorage`·`quizService` 가 나오는지 본다(주석은 걷어낸다).

```
→ 파일을 옮기거나 이름을 바꾸면 그 FILES 배열도 같이 고친다.
  안 고치면 가드가 **없는 파일을 지키면서 초록**이 된다. 그게 이 종류 테스트의 유일한 고장 방식이다.
```

**점수는 넘긴 뒤에 다시 시작한다.** 마지막 카드에서 퀴즈로 넘어가는 버튼이 있고(앞 10장),
거기서부터는 정상 퀴즈라 기록이 남는다. 기존 `retryWordIds` 통로를 그대로 쓴다.

#### 저장하는 것은 취향 하나뿐

```
@my_word_flashcard_prefs   순서 · 앞면이 단어인가 · 발음 자동재생 · 마지막 카테고리
```

🔴 **`BACKUP_KEYS` 에 일부러 넣지 않았다.** 학습 기록이 아니라 이 기기에서의 보기 취향이고,
백업에 담으면 복원한 기기의 화면이 조용히 바뀐다. 만기순 정렬은 `getDueWordIds` 로 **읽기만** 한다.

#### ⚠ 홈 첫 줄이 두 칸에서 세 칸이 됐다

`학습하기 · 플래시카드 · 단어 추가`. 360 기준 칸 너비가 154 에서 **99** 로 줄어 안쪽 여백과
아이콘을 같이 줄였고 부제를 지웠다. 🚫 `adjustsFontSizeToFit` 은 쓰지 않는다(iOS 전용이다).
제목은 `numberOfLines={2}` 로 두 줄까지 받는다.

⚠ 단어장 안이 아니라 **홈**에 둔 것은 사용자 결정이다. 만드는 비용으로는 단어장이 쌌지만,
사용자가 보는 자리는 **화면의 역할**이 정한다(단어장은 관리하는 곳이지 학습하는 곳이 아니다).

#### 🔴 여기서 잡은 버그: 타입 체크와 테스트 284개를 전부 통과한 채 화면이 비어 있었다

카드를 넘기면 **카운터는 2/12 로 올라가는데 카드가 안 보였다.**

```
원인   애니메이션을 useNativeDriver 로 돌린 뒤 setValue 로 위치를 되돌렸다.
       네이티브로 넘긴 변환은 네이티브 뷰가 값을 들고 있어서, setValue 는 JS 쪽 숫자만 바꾼다.
       화면의 카드는 화면 밖에 그대로 서 있었다.
증상   크래시 없음 · 경고 없음 · 타입 통과 · 테스트 284개 전부 통과
```

🔴 **손가락으로 끄는 반응이 죽은 것도 같은 원인이었다.** 그래서 이 화면의 가로 이동은
**전부 JS 드라이버(`useNativeDriver: false`)로 쓴다.** 카드 뒤집기(`FlipCard`)는 `setValue` 를
쓰지 않는 순수 애니메이션이라 네이티브 드라이버가 맞다. **둘을 섞어 쓰지 마라.**

⚠ 교훈은 **"애니메이션은 화면으로 본다"** 이다. 값이 맞는지를 재는 테스트는 이걸 못 잡는다.
값은 내내 맞았고 틀린 것은 **그 값을 누가 들고 있느냐**였다.
...
경고가 사라지는가.~~ → ✅ **사라졌다**(바로 위 게시 절). 2026-09-18 문서 감사에서 이 줄만 열린 채 남아 있던 것을 닫았다.
...
- ~~🔴 **`mapping.txt` 를 함께 업로드한다** — `android/app/build/outputs/mapping/release/mapping.txt`.~~
  → 따로 올릴 필요가 없었다. **gradle 이 AAB 안에 넣는다**(위 게시 절 · 아래 산출물 절 실측). 별도 업로드 절차를 만들지 않는다.
  (2026-09-18 문서 감사에서 정정. 이 준비 절과 게시 절이 서로 반대로 말하고 있었다)
...
→ ✅ **2026-09-14 1.7.0 릴리스 빌드에서 검증했다**(아래 1.7.0 절).
...
~~🔴 **검증이 남았다. 다음 릴리스를 구울 때 반드시 한다.**~~ → ✅ **2026-09-14 1.7.0 릴리스 빌드에서 검증했다**(아래 1.7.0 절 · 알림 아이콘 실물 확인).
아래는 그때 무엇을 봐야 했는지의 기록이다. 이 옵션은 우리 AGP 에서 Experimental 등급이고, 리소스를 더 공격적으로 지운다.
...
#### ✅ 1.7.0 릴리스 빌드 E2E 통과 · 검토 제출 (2026-09-14)

전용 AVD `my_word`(5576)에서 릴리스 APK 로 확인했다. 이번 릴리스에 **최적화된 리소스 축소(`optimizedShrinking`)가 처음 실렸다.**
값은 화면을 보기 전에 계산해 두었다(단어 13 · 기록 9건 중 정답 7 → 77.8% · `토익 필수` 12장 · 퀴즈 넘김 10개).

| 확인 | 근거 |
|---|---|
| 서명 | APK·AAB SHA-256 `EE:68:31:DB:…:86:1F:5B` = 게시된 vc21 과 같은 키 |
| 🔴 알림 아이콘 | `aapt2`: `0x7f0800f8 = drawable/notification_icon` · 매니페스트 두 줄이 그 번호 · PNG 5장이 vc21 과 **바이트 크기까지 같다**(244·347·404·586·766) · 실제 발화 `icon=0x7f0800f8` · 알림창에서 **흰 네모 아님** · `vis=PRIVATE` |
| `expo-updates` | `dev.expo.EASSharedPreferences.xml`(`eas-client-id`) · `updates.db` |
| 기타 모듈 | `SecureStore.xml` 키 2개 · `admob.xml` · 알림 저장소 |
| 🔴 플래시카드 채점 안 함 | 12장 넘기기 전후 `@my_word_quiz_results` · `@my_word_srs` md5 **그대로** |
| 밀어 넘기기 | 9-10 에 카드가 사라지던 동작이 R8 뒤에도 재발 없음(`2 / 12` 가 보인다) |
| 오프라인 첫 실행 | 비행기 모드 · 오류 문구 **0건**(uiautomator) |

콘솔 실측: 신규 설치 **15.7MB(-195KB)** · 업데이트 5.63MB · 지원 기기 감소 **0대**(전화 12,477 · 태블릿 6,684 · TV 4).
2026-09-14 10:37 검토 제출(전체 출시 · 145개국). 올린 파일은 `D:\builds\my_word\myword-vc22.aab`.

🔴 **알림 발화 시험에서 헛걸음 둘. 다음에는 처음부터 이 순서로 한다.**

```
① 앱을 화면에 띄워 둔 채 터뜨리면 알림이 안 뜬다. 결함이 아니라 설계다
   notificationService 가 앞에 떠 있을 때 shouldShowBanner · shouldShowList 를 false 로 준다
   (지금 앱을 쓰고 있는 사람에게 복습 알림은 필요 없다)
   → 반드시 HOME 으로 내린 뒤 터뜨린다
   ⚠ 예약 수는 7→6 으로 줄어 있어서 "발화했는데 알림이 없다" 로 헷갈린다
② 부정확 알람(window=+1h)이라 시계를 00:00 에 딱 맞추면 창 안이라 안 뜬다
   → 창 끝 너머(01:00:30)로 옮기고 TIME_SET 을 쏜다
```

⚠ `aapt2 dump resources` 를 `drawable/notification_icon ` 이름으로 grep 하니 **0줄**이 나왔다(검색식 문제).
축소가 지운 줄 알 뻔했다. **매니페스트의 리소스 번호(`0x7f0800f8`)로 찾는다.** 0 이 나오면 세는 방법부터 의심한다.

🔴 **새 버전 2단계 `저장` 은 이번에 전송이 아니었다.** 저장 뒤 *"게시 개요로 이동하시겠어요?"* 창이 뜨고 제출 대기로 남았다.
전송은 게시 개요의 `검토를 위해 변경사항 N개 제출` 이 했다. 조각(9-09)에서는 저장 즉시 전송이었고 원인은 확인 안 됨이다.
→ **판정은 기억이 아니라 게시 개요의 상태 문구로 한다**(`common/PRE_LAUNCH_CHECK.md` §4 에도 정정을 넣었다).

⚠ 출시 노트 검수에서 **번역이 원문보다 좁게 옮겨진 줄**을 잡았다. `좌우로 밀면 다음 단어로 넘어갑니다` 는 틀렸다
(오른쪽으로 밀면 이전 카드다). 영어 `move between words` 는 맞았다. **노트는 코드 동작과 한 줄씩 대조한다.**
...
✅ **닫혔다**: 2026-09-14 OTA 직전에 굽고 검사했고(CHANGELOG 1.7.0 OTA 줄), 2026-09-18 OTA 직전에도
`npx expo export --platform android --clear` → `common/tools/storeshots/scan_ota_bundle.py` 로 다시 했다.
둘 다 걸린 것은 `localhost` 1건(라이브러리 문자열이 붙은 것, 스킴 · 포트 없음)과 서버 주소 오탐뿐이었다.
**OTA 를 낼 때마다 이 두 명령을 먼저 돌린다.** 아래는 처음 못 했던 날의 기록이다.

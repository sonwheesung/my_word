...
//
// ✅ **그래서 이건 사람이 기억할 규칙이 아니라 기계가 잡는다** — `npm run check:tests`
//    (`scripts/check-tests.mjs`)가 **통과 개수**의 바닥값 · 스위트 수 · 스킵 0 · "알려진 고장" 목록을 본다.
//    바닥을 `numTotalTests` 가 아니라 `numPassedTests` 로 잡는 이유는 그 파일에 적어 뒀다 —
//    jest 는 **스킵된 테스트도 total 에 세기 때문에** `.skip` 하나로는 total 이 안 줄어든다.
//    ⚠ 목을 더해 스위트를 살리거나 테스트를 추가했으면 **그 파일의 MIN_PASSING 도 같이 올린다.**
//    귀찮은 것이 요점이다 — 안 올리면 다음에 스위트가 죽어도 안 걸린다.
// ─────────────────────────────────────────────────────────────────────────────

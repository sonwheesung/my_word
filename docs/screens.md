...
- 기기 뒤로가기도 `나가기` 와 같다. 같은 확인창을 띄우고 같은 곳으로 간다(App.tsx `exitQuiz`)
  ~~예전에는 확인 없이 나갔고, 복습 배너에서 왔을 때 퀴즈 설정으로 갔다~~ → 2026-09-18 고침 · 에뮬레이터 확인
...
✅ **2026-09-18 고친 것** (릴리스 빌드 · 에뮬레이터 확인. 증거 `D:/builds/my_word/evidence/2026-09-18/` 5~8번)
- 스크롤: `content` 를 `flexGrow: 1` 로. 9개 틀린 판에서 점수부터 `홈으로` · `틀린 9개 다시 풀기` · `전체 다시 풀기` 까지 닿는다
- 다시 풀기: App 이 목록으로 시작한 퀴즈의 **처음 목록**(`quizSourceWordIds`)을 들고 있다가 다시 낸다. 복습 배너 · 플래시카드 둘 다 `1 / 10` 으로 다시 시작
- 영어 라벨: `Correct answer` · `Your answer` 가 한 줄로 나온다(`minWidth` + `flexShrink: 0`)

~~고치기 전 기록~~ (2026-09-18 에뮬레이터에서 본 것. 원인 설명으로 남긴다)
- 🔴 **틀린 단어가 많으면 화면이 스크롤되지 않았다.** `contentContainerStyle` 이 `flex: 1` + `justifyContent: 'center'` 라
...
- 복습 배너 · 플래시카드에서 온 퀴즈에서 `다시 풀기` · `전체 다시 풀기` 를 누르면 **홈이 떴다.** 단어 목록을 비우는데 카테고리도 없어 퀴즈를 못 그렸다.
  화면 상태는 퀴즈로 남아, 그 홈에서 뒤로가기를 누르면 종료 확인 대신 **퀴즈 설정**이 열렸다
